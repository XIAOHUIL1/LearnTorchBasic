...
def get_activation(name="silu", inplace=True):
    if name == "silu":
        ...
        module = nn.AttributeError("Unsupported act type:{}".format(name))
    return module
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, shortcut=True, expansion=0.5, depthwise=False, act="silu", ):
        ...
        # -----------------------------#
        #  利用1x1卷积进行通道数的缩减。缩减率一般为50%
        # -----------------------------#
        self.conv1 = BaseConv(in_channels, hidden_channels, 1, stride=1, act=act)
        # ------------------------------------------------------#
        # 利用3x3的卷积进行通道数的扩展==扩张，一般为50%。并且完成特征提取
        # ------------------------------------------------------#
...
class CSPDarknet(nn.Module):
    def __init__(self, dep_mul, wid_mul, out_features=("dark3", "dark4", "dark5"), depthwise=False, act="silu", ):
        ...
        # 完成CSPLayer：80,80,256->80,80,256
        # ------------------------------------#
        ...
        # 完成CSPLayer：40,40,512->40,40,512
        # ------------------------------------#
        ...
        # 完成SPPBottleneck之后：20,20,1024-> 20,20,1024
        # 完成CSPLayer：20,20,1024 -> 20,20,1024
        # ------------------------------------#
    ...
    def forward(self, x):
        ...
        outputs["dark2"] = x
        # ------------------------------------#
        # dark3 输出80,80,256 是一个有效特征层
        # ------------------------------------#
        x = self.dark3(x)
        outputs["dark3"] = x
        # ------------------------------------#
        # dark4 输出40,40,512 是一个有效特征层
        # ------------------------------------#
        x = self.dark4(x)
        outputs["dark4"] = x
        # ------------------------------------#
        # dark5 输出20,20,1024 是一个有效特征层
        # ------------------------------------#
        x = self.dark5(x)
        outputs["dark5"] = x
